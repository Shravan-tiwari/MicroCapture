		...
		while (true)
		{
			...
			if (loop)
			{
				...
				std::cout
					...
					<< "[101] Edit Owner Name \n"
					<< "[102] Edit Artist Name \n"
					<< "[103] Edit Date / Time \n"
					<< "[104] Edit Daylight Saving Time \n"
					<< "[105] Edit Zone Setting \n"
					<< std::endl;
				std::cout << "-------------------------------" << std::endl;
			...
			if (keyflag == true) // Within this scope, the CheckKey thread is terminated
			{
				...
				if (std::regex_search(control_number, match_results, std::regex("[0-9]")))
				{
					...
					// Download CSD File
					else if (control_number == "38")
					{
                        for (i = 0; i < _openedCamerasArray.size(); i++)
                        {
                            _openedCamerasArray[i]->GetCsdFileData();
                        }
						pause_return();
						clr_screen();
						loop = true;
					...
					// Write CSD File
					else if (control_number == "39")
					{
						for (i = 0; i < _openedCamerasArray.size(); i++)
						{
							_openedCamerasArray[i]->UILock();
                            _openedCamerasArray[i]->SetCsdFileData();
					...
					else if (control_number == "44")
					{
						...
						//set MicVolume
						data = getvalue();
						if (data != -1 || data < MovieRecVolume_table.size()) //Since it is input by index, it is set to "<".
						{
							for (i = 0; i < _openedCamerasArray.size(); i++)
							{
					...
					else if (control_number == "62")
					{
						...
						if (apldata.apertureLockStatus == 0 || apldata.apertureLockStatus == 1)
						{
							...
							{
								std::cout << "--------------------------------" << std::endl;
								// AV display (displayed on each camera)
								for (i = 0; i < _openedCamerasArray.size(); i++)
									_openedCamerasArray[i]->GetPropertyDesc(kEdsPropID_Av, av_table);

								std::cout << "input no." << std::endl;
								std::cout << ">";
								int inputNo = getvalue(); // Receives the display number (1-based) seen by the user

								if (inputNo == 0xffffffff) {
									std::cout << "error invalid parameter." << std::endl;
									pause_return();
									clr_screen();
									loop = true;
								...
								// Important: SetApertureLock expects a 0-based index, so set it to -1.
								apldata.avValue = static_cast<EdsUInt32>(inputNo - 1);
							}
							else
							{
								// If the lock is OFF, the SetApertureLock side will get the current value, so enter a sentinel or set it to 0xffffffff
								apldata.avValue = 0xffffffff;
							}
					...
					else if (control_number == "70")
					{
						...
						if (data != -1 || data <= AutoPowerOffSetting_table.size())
						{
							for (i = 0; i < _openedCamerasArray.size(); i++)
							{
					...
					// Edit Owner Name
					else if (control_number == "101")
					{
						std::string str(64, 0);
						for (i = 0; i < _openedCamerasArray.size(); i++)
						{
							_openedCamerasArray[i]->GetPropertyvalue(kEdsPropID_OwnerName);
							std::cout << " \n"
								<< std::endl;
						}
						std::cout << "What do you want?\n"
						...
						if (std::regex_search(str, match_results, std::regex("e", std::regex_constants::icase)))
						{
							std::cout << "enter Owner Name. (MAX 64 ASCII characters)\n"
								<< std::endl;
							std::cout << ">";
							err = getstrings(str, 64);
							if (err == 1)
							{
								...
								{
									_openedCamerasArray[i]->UILock();
									_openedCamerasArray[i]->SetPropertyValue(kEdsPropID_OwnerName, str);
									_openedCamerasArray[i]->UIUnLock();
								}
							}
							pause_return();
					...
					// Edit Artist Name
					else if (control_number == "102")
					{
						std::string str(64, 0);
						for (i = 0; i < _openedCamerasArray.size(); i++)
						{
							_openedCamerasArray[i]->GetPropertyvalue(kEdsPropID_Artist);
							std::cout << " \n"
								<< std::endl;
						}
						std::cout << "What do you want?\n"
						...
						if (std::regex_search(str, match_results, std::regex("e", std::regex_constants::icase)))
						{
							std::cout << "enter Artist Name. (MAX 64 ASCII characters)\n"
								<< std::endl;
							std::cout << ">";
							err = getstrings(str, 64);
							if (err == 1)
							{
								...
								{
									_openedCamerasArray[i]->UILock();
									_openedCamerasArray[i]->SetPropertyValue(kEdsPropID_Artist, str);
									_openedCamerasArray[i]->UIUnLock();
								}
							}
							pause_return();
					...
					// Edit Date / Time 
					else if (control_number == "103")
					{
						EdsTime utcTime;
						std::string str(64, 0);
						for (i = 0; i < _openedCamerasArray.size(); i++)
							_openedCamerasArray[i]->GetPropertyvalue(kEdsPropID_UTCTime);

						std::cout << "Do you edit Date / Time ?\n"
							<< std::endl;
						std::cout << "[Y]es? or [N]o?\n"
							<< std::endl;
						std::cout << ">";
						...
						if (std::regex_search(str, match_results, std::regex("y", std::regex_constants::icase)))
						{
							std::cout << "enter Date / Time. (ex : 2019/09/12 09:11)\n"
								<< std::endl;
							std::cout << ">";
							err = getstrings(str, 64);
							if (err == 1)
							{
								...
								{
									_openedCamerasArray[i]->UILock();
									_openedCamerasArray[i]->SetPropertyValue(kEdsPropID_UTCTime, str);
									_openedCamerasArray[i]->UIUnLock();
								}
							}
							pause_return();
					...
					else if (control_number == "104")
					{
						EdsInt32 data;
						for (i = 0; i < _openedCamerasArray.size(); i++)
							_openedCamerasArray[i]->GetPropertyvalue(kEdsPropID_SummerTimeSetting);

						std::cout << "Do you want to set Daylight Saving Time to [0]OFF or [1]ON ?\n"
							<< std::endl;
						std::cout << ">";
						...
						if (data == 0 || data == 1) {
							for (i = 0; i < _openedCamerasArray.size(); i++)
							{
								_openedCamerasArray[i]->SetPropertyValue(kEdsPropID_SummerTimeSetting, &data);
							}
							pause_return();
						}
						clr_screen();
					...
					// Edit TimeZone Setting
					else if (control_number == "105")
					{
						for (i = 0; i < _openedCamerasArray.size(); i++)
							_openedCamerasArray[i]->GetProperty(kEdsPropID_TimeZone, TimeZone_table);
						for (i = 0; i < _openedCamerasArray.size(); i++)
							_openedCamerasArray[i]->GetPropertyDesc(kEdsPropID_TimeZone, TimeZone_table);

						std::cout << "input no." << std::endl;
						std::cout << ">";

						data = getvalue();
						if (data != -1 || data <= TimeZone_table.size())
						{
							for (i = 0; i < _openedCamerasArray.size(); i++)
							{
								_openedCamerasArray[i]->SetProperty(kEdsPropID_TimeZone, data, TimeZone_table);
								_openedCamerasArray[i]->setTv(data);
							}
							pause_return();
						}
