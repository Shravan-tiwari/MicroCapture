...
protected:
	...
	EdsUInt32 _bodyID;
	EdsChar _bodyIDEx[EDS_MAX_NAME];
...
public:
	...
	std::string EdsTime2StrTime(EdsTime edsTime);
	bool tryParseExact(const std::string& inputTime, std::tm& outputTime);
    ...
    EdsError GetCsdFileData();
    EdsError SetCsdFileData();
};
