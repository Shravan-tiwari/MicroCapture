...
paper_bad = [(p < 0.10 or p > 0.75) for p in PAPER]
...
1. **Left-side blob on images 90/96/117/118** — a hand or wall region survives as a
   second component attached to the paper mask. Needs a shape/position test in Stage 3.
2. **Images 17, 18 (black covers)** — paper fraction 0.05/0.075; only the bright title
   strip is caught. Brightness cannot segment a black cover against a dark stand. These
   are single closed books, not open spreads, and may be out of scope — to confirm with
   the operator.
3. **Fore-edge block is included in the paper mask.** Correct at this stage: the mask is
   *paper*, and the block *is* paper. Separating the front page from the block is
   Stage 3's job (the operator's "front pages, not side pages" requirement).
